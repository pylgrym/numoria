...
extern class_type classs[MAX_CLASS];
...
#ifndef MAC
extern int (*store_buy[MAX_STORES])(int); // JG: I added int here.
#endif
...
#if defined(MSDOS) || defined(PC_JG)
...
int next_to_walls(int, int); // JG: I added this, only appears in .. VMS section?
...
int inven_damage(int(*)(inven_type*), int);
...
int set_large(treasure_type*); //  inven_type *); // That was wrong: it's treasure_type, NOT inven_type.. which also explains why it would give compile error..(but behave correctly, because they actually matched anyway..)
...
void get_flags(int typ, int32u *weapon_type, int *harm_type, int(**)(inven_type*));
...
#ifdef unix
/* unix.c */
int check_input(int); // unix variant.
#if 0
int system_cmd(char *);
#endif
void user_name(char *);
...
#ifdef ATARI_ST
/* atarist.c */
int check_input (); // another atari variant.
void user_name ();
int access ();
void chmod ();
#endif
...
#ifdef unix
/* unix.c */
int check_input(); // another unix variant.
#if 0
int system_cmd();
#endif
void user_name();
