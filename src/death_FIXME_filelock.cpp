/*
has problems with lock filelock, L_SET; L_INCR, LOCK_EX.
*/
...
char *day)
{
  ...
  time_t clockvar;
#else
  time_t clockvar; // long 
#endif

#ifdef MAC
  clockvar = time((time_t *) 0);
#else
  clockvar = time((time_t *)(long *)0);
#endif
  tmp = ctime(&clockvar);
  tmp[10] = '\0';
  (void) strcpy(day, tmp);
	      ...
	      )
	    {
	      (void) sprintf(string,
			   "%-4d%8ld %-19.19s %c %-10.10s %-7.7s%3d %-22.22s",
			     ...
			     race[score.race].trace, classs[score.classs].title,
...
static void highscores()
{
  ...
  int8u version_maj, version_min, patch_level;
  long curpos;
#if defined(VMS) || defined(MSDOS) || defined(AMIGA) || defined(MAC) || defined(APOLLO) || defined (PC_JG)
  char string[100];
#endif
...
#if defined(MSDOS) || defined(VMS) || defined(AMIGA) || defined(MAC) || defined (PC_JG)
#if defined(MAC) || defined(MSDOS)
  if ((highscore_fp = fopen(MORIA_TOP, "rb+")) == NULL)
#else
  if ((highscore_fp = fopen(MORIA_TOP, "r+")) == NULL)
	...
	break;
	 ...
	 entry is "(saved)" */
		...
		|| (new_entry.uid == 0 &&!strcmp(old_entry.died_from,"(saved)")
	       ...
	       && new_entry.classs == old_entry.classs)
	{
#if defined(MSDOS) || defined(VMS) || defined(AMIGA) || defined(MAC) || defined(APOLLO)
	  (void) fclose (highscore_fp);
#endif
