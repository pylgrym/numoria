...
/*
// Cloned from ibmpc\ms_misc.c
// 80
#define PATHLEN	256
char	moriatop[PATHLEN]="nothing yet";
char	moriasav[PATHLEN]="nothing yet";

void setFilePathsJG() {
	// Inspired by ibmpc\ms_misc.c: msdos_init():
	const char* basePath = ".\\files\\";
	(void)strcpy(moriatop, basePath);
	(void)strcat(moriatop, MORIA_TOP_NAME);
	(void)strcpy(moriasav, basePath);
...
void init_scorefile()
{
  ...
  if (highscore_fp == NULL)
      ...
      if (highscore_fp == NULL)
	{
	  ...
	  (void)jgprintf(stderr, "Can't open score file \"%s\"\n", MORIA_TOP);		
	  exit(1);
#endif
    }
...
void print_objects()
{
  ...
  if (!get_string(tmp_str, 0, 27, 10))
    return;
  nobj = atoi(tmp_str);
  smalll = get_check("Small objects only?");
  if ((nobj > 0) && (level > -1) && (level < 1201))
...
#endif
	{
	  ...
	  if ((file1 = fopen(filename1, "w")) != NULL)
		...
		{
		  invcopy(&t_list[j], 
			  sorted_objects[get_obj_num(level,smalll)]);
...
// FIXME: 
// enum missingEnum1 { O_WRONLY, O_CREAT, O_EXCL };
//int open(const char* filename, int, int);
// void close(int);
...
#endif
{
  ...
  if (!access(filename1, AREAD))
    ...
    fd = creat (filename1, 1);
#else
  fd = _open (filename1, O_WRONLY|O_CREAT|O_EXCL, 0644);
  if (fd < 0 && errno == EEXIST)
    {
      (void) sprintf(out_val, "Replace existing file %s?", filename1);
      if (get_check(out_val))
	fd = _open(filename1, O_WRONLY, 0644);
    }
#endif
#endif
  if (fd >= 0)
