...
//static char *stat_names[] = { "STR : ", "INT : ", "WIS : ", "DEX : ", "CON : ", "CHR : " };
static char *stat_names[] = { "STR", "INT", "WIS", "DEX", "CON", "CHR" };
...
int get_obj_num(int level, int must_be_small)
...
void place_object(int y, int x, int must_be_small)
...
void alloc_object( // alloc_set, typ, num)
  int(*alloc_set)(int), // JG: I add (int) here
  int typ, int num)
{
  register int i, j, k;
...
// int stat;
{
  ...
  if (stat == A_STR)
  {
    ...
    calc_bonuses();
  else if (stat == A_INT && classs[py.misc.pclass].spell == MAGE)
  {
    calc_spells(A_INT);
    calc_mana(A_INT);
  }
  else if (stat == A_WIS && classs[py.misc.pclass].spell == PRIEST)
  {
    calc_spells(A_WIS);
    calc_mana(A_WIS);
  }
...
void put_character()
{
  ...
  if (character_generated)
  {
    ...
    put_buffer(classs[m_ptr->pclass].title, 5, 15);
  }
}
...
void put_misc3()
{
  ...
  vtype xinfra;
  register struct player_type::misc *p_ptr;
...
void get_name()
{
  ...
  {
    strncpy(py.misc.name, "pilgrim", sizeof py.misc.name); // user_name(py.misc.name);
...
// int spell;
{
  ...
  if (classs[py.misc.pclass].spell == MAGE)
    stat = A_INT;
  else
    stat = A_WIS;
  ...
  int comment, int nonconsec)
{
  ...
  if (comment)
    ...
    col = 31;
  offset = (classs[py.misc.pclass].spell == MAGE ? SPELL_OFFSET : PRAYER_OFFSET);
  erase_line(1, col);
...
// int stat;
{
  ...
  levels = p_ptr->lev - classs[p_ptr->pclass].first_spell_lev + 1;
...
void gain_spells()
{
  ...
  p_ptr = &py.misc;
  msp_ptr = &magic_spell[p_ptr->pclass - 1][0];
  if (classs[p_ptr->pclass].spell == MAGE)
  {
    stat = A_INT;
    offset = SPELL_OFFSET;
...
void prt_experience()
{
  ...
  while ((p_ptr->lev < MAX_PLAYER_LEVEL) &&
    (player_exp[p_ptr->lev - 1] * p_ptr->expfact / 100) <= (int32u)p_ptr->exp)
    gain_level();
...
void insert_str( // object_str, mtc_str, insert)
  char *object_str, char *mtc_str, char *insert)
{
  int obj_len;
  char *bound, *pc;
...
int critical_blow( // weight, plus, dam, attack_type)
  register int weight, int plus, int dam,
  int attack_type)
{
