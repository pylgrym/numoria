...
int32 offer, int32 asking, int final)
{
  vtype comment;
...
int store_num,
char *comment,
register int32 *new_offer, int32 last_offer,
int num_offer, int factor)
{
  register int flag;
...
#else
      flag = (*store_buy[store_num]) (inventory[counter].tval); // calling with one int8u.
#endif
      mask[counter] = flag;
      if (flag)
	{
