...
// JG, HACK:
extern void sleep(int seconds); // Please implement me.
int kill(int, int){ return -1;  }
// #define PC_JG 1
...
/*ARGSUSED*/
#if !defined(USG) && !defined(PC_JG)
	#ifdef __386BSD__
		static void signal_handler(sig, code, scp)
	#else
		static int signal_handler(int sig, int code, struct sigcontext *scp)
	#endif
	{
	  int smask;
...
#else
#if defined(__TURBOC__) || defined(AMIGA) || defined(PC_JG)
...
// int sig;
{
  ...
  if(error_sig >= 0)	/* Ignore all second signals. */
	...
	(void) MSIGNAL(sig, SIG_DFL);
	  return; // 0; // JG: Maybe 0 is not a good value to return?
    }
  error_sig = sig;

  /* Allow player to think twice. Wizard may force a core dump. */
  if (sig == SIGINT
#if !defined(MSDOS) && !defined(AMIGA) && !defined(ATARIST_TC) && !defined(PC_JG)
      || sig == SIGQUIT
#endif
      )
    {
      ...
      else if (!character_saved && character_generated)
	{
	  if (!get_check("Really commit *Suicide*?"))
		...
		disturb(1, 0);
	      ...
	      // (void) sigsetmask(smask); // JG,FIXME: I don't know what all this should be on PC.
		...
		put_buffer(" -more-", MSG_LINE, 0);
	      put_qio();
		  return; // 0;		/* OK. We don't quit. */ // JG: Maybe 0 is not a good value to return?
	    }
...
{
#ifdef PC_JG
  return; // I don't think I want any of this signal-stuff,
  // until I find out I need any part of it..
  /* as far as I can tell, the main purpose is,
  in the event of unexpected 'crashing/quitting' behaviour,
  that the system will try to save/recover the character status,
  so the player doesn't lose his save-file.
    The reason for this, is the anti-cheating tamper-protection:
   - If you 'die unexpectedly', your save-file will be left in
   'gone out for playing' mode, and you won't be able to continue playing.
  */
#endif // PC_JG
...
#if defined(AMIGA) || defined(PC_JG)
/*  (void) MSIGNAL(SIGINT, signal_handler); */
