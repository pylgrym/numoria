...
int  clear() { return Csr.clear(); }
int    overwrite(const WINDOW *src, WINDOW *tgt) { return Csr.overwrite(src,tgt); }
int    clrtoeol() { return Csr.clrtoeol(); }
int    clrtobot() { return Csr.clrtobot(); }
void   getyx(WINDOW *win, int& y, int& x) { Csr.getyx(win, y, x); }
int touchwin(WINDOW *win) { return Csr.touchwin(win); }
WINDOW *initscr() { return Csr.initscr(); }
WINDOW *newwin(int nlines, int ncols, int begin_y, int begin_x) { return Csr.newwin(nlines,ncols,begin_y, begin_x); }
int getch() { return Csr.getch(); }

//  WINDOW   *stdscr=NULL, *curscr=NULL;
// extern int LINES=25;
// extern int COLS=80;

// NON_CURSES functions - 
void sleep(int seconds) {
  Sleep(seconds * 1000);
}
// int write(int, const char* str, int) { return -1;  }// ONLY USED TO write BELL RING sound BEEP  "\007", 1);
...
#if !defined(MAC) && !defined(MSDOS) && !defined(ATARI_ST) && !defined(VMS) && !defined(PC_JG)
	#ifndef AMIGA
		#ifdef USG
			#ifdef __linux__
...
void jgprintf(void* fileOrBuf, const char * format, ...) {
	// FIXME - make this print/display an error:
	int a = 0;
	int b = 0;
	assert(false);
	int c = a / b;
...
#else
{
...
#ifdef PC_JG
  "init_curses, no need for ioctl crap for Jakobs PC.";
#else
	#ifndef USG
  ...
  if (initscr() == ERR)
#endif
    {
      ...
      (void)jgprintf(NULL, "Error allocating screen in curses package.\n");
      exit(1);
    }
  if (CursesJG::LINES < 24 || CursesJG::COLS < 80)	 /* Check we have enough screen. -CJS- */
    {
      // (void)printf("Screen too small for moria.\n");
      (void)jgprintf(NULL, "Screen too small for moria.\n");
      exit (1);
    }
#ifdef SIGTSTP
#if defined(atarist) && defined(__GNUC__)
  ...
  if (((savescr = newwin (0, 0, 0, 0)) == NULL)
...
#endif
    {
      ...
      (void)jgprintf(NULL, "Out of memory in starting up curses.\n");
      exit_game();
    }
  (void) clear();
  (void) refresh();
...
void moriaterm()
#if defined(MAC) || defined(PC_JG)
/* Nothing to do on Mac  OR PC_JG */
{
}
#else
{
...
#else
#if defined(AMIGA) || defined(ATARIST_TC) || defined(PC_JG)
{
  put_buffer("This command is not implemented.JG.shell_out\n", 0, 0);
}
#else
...
void flush() // Purpose of flush is to PROTECT the player by throwing away buffered key-presses, if he's run into e.g. a monster.
#ifdef MAC
{
...
#else
{
  ...
  if (!eof_flag) { // What we really need here, is a PeekMessage loop,
    // to sleep for 10 x 50 ms, and soak up any keys pressed.
    extern void keyEaterLoopJG();
    keyEaterLoopJG();
    // while (check_input(0));
  }
#endif
...
void bell()
{
  ...
  FlashWindow(NULL, true);
  extern void flashWndJG();
  flashWndJG();
...
int check_input(int microsecs) { 
  return false; // FIXME, flush() should throw away extra keys.
  /* JG: This (or flush) needs to be a message loop 
  that eats key-presses and waits for a little while..
  */
}
