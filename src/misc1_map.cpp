...
#else
    clock_var = (int32u)time((time_t *)(long *)0);
#endif
  else
    clock_var = seed;
...
void prt_map()
{
  /* NB! - this part is where we could move the map left,
  if dashboard was put at bottom,
  HOWEVER, it requires that ALL 'map' code respects a moved  [panel_col_min/panel_col_max]..
  (ie all small delta drawing actions.)
  */
  register int i, j, k;
  LocInf tmp_char(' ',Ti_Empty); // register unsigned char tmp_char;
...
{
  register struct player_type::flags *p_ptr;
...
int place_monster(   register int y, int x, int z,  int slp)
{
